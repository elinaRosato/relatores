...
<div class="delay-section">
  ...
  <div class="presets">
    ...
    {#each presets as p (p)}
      <button class="preset-btn" class:active={$delaySeconds === p} onclick={() => setDelay(p)}>
        {p}s
      </button>
    {/each}
  </div>
  ...
  <div class="fine-tune">
    ...
    <button class="fine-btn" onclick={() => setDelay($delaySeconds - 0.1)} aria-label="Restar 0.1 segundos">−</button>
    <span class="fine-step">0.1s</span>
    <button class="fine-btn" onclick={() => setDelay($delaySeconds + 0.1)} aria-label="Sumar 0.1 segundos">+</button>
  </div>
</div>
